...
void ScanStart( const std_msgs::String& msg) {
    std::stringstream stream;
    stream.str(msg.data);
    echo_msg.data = msg.data;
    stream>>receiveData;
    stream.str("");
    if(receiveData == 1) scan_flag = 1;
    else if (receiveData == 0) scan_flag = 0;
    Echo.publish(&echo_msg);
}

char echo_arr[20];
void FindCircle( const std_msgs::String& msg) {
    std::stringstream stream;
    stream.str(msg.data);
    echo_msg.data = msg.data;
    stream>>receiveData;
    stream.str("");
    if(receiveData == 2) Circle_flag = 2;
    else if (receiveData == 3) Circle_flag = 3;
    Echo.publish(&echo_msg);
}


ros::Subscriber<std_msgs::String> sub1("Scan/start", ScanStart );
ros::Subscriber<std_msgs::String> sub2("FindCircle", FindCircle );
...
void Find_Hall(const char **l){
  ...
  for(int i =0 ; i<60; i++){
    ...
    delay(100);
    if(i==30) Circle_flag = 3;
    int32_t get_data = 0;
    if(Circle_flag==3){
        for(int idx = 0; idx <  sizeof(id_list)/sizeof(char);idx++){
          result = dxl_wb.itemRead(id_list[idx], "Present_Position", &get_data, l);
          if (result == false)
          {
            Serial.println(*l);
          ...
          else
          {
            ...
            Serial.print(idx); Serial.print("Motor Pos = ");
            Serial.println(get_data);
            pos_list[idx] = Position(get_data);
          }
        }
        
        for(int idx =0; idx <  sizeof(id_list_2horn)/sizeof(char); idx ++){
          result = dxl_wb.itemRead(id_list_2horn[idx], "Present_Position", &get_data, l);
          if (result == false)
          {
            Serial.println(*l);
            Serial.println("Failed to get present position");
          ...
          else
          {
            //Serial.print("Succeed to get present position(value : ");
            //Serial.print(get_data);
            Serial.print(idx); Serial.print("2축 Motor Pos = ");
            Serial.println(Position(get_data));
            pos_list_2horn[idx] = Position(get_data);
          }
        }
