    ...
    while (1) {
        if (serialDataAvail(fd)) {
            ...
            if (1) {
                ...
                memset(temp, 0, sizeof(temp));
                sprintf(temp, "insert into parkinglot_log(ID,CARNUM,SPOT,INOUT) values (null,'%s','%s',%d)", number_back, rest_spot,inout);
                ...
                char send_msg[40] = "";
                sprintf(send_msg, "%d,%s@%d@%sL", inout,number_back, type, rest_spot);
                int i = 0;
