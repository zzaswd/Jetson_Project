        ...
        if (friend_count >= 6) {
            ...
            if ((boundRect2[select].tl().y - 20 > 0) && (boundRect2[select].tl().x - 20 > 0) && (boundRect2[select].tl().y + 20 + plate_width * 0.3 < 480) && (boundRect2[select].tl().x + plate_width + 40 < 640)) {
                ...
                */
                ...
                //cv::adaptiveThreshold(test, test_thresh, 255, cv::ADAPTIVE_THRESH_GAUSSIAN_C, cv::THRESH_BINARY_INV, 19, 9);
                cv::Mat Merge_Hist[] = { hist_thresh,hist_thresh,hist_thresh };
                cv::Mat hist_merge;
                ...
                //==============================
                cv::Mat img_dilate;
                cv::dilate(hist_thresh, img_dilate, cv::Mat());
                ...
                cv::erode(img_dilate, img_erode, cv::Mat::ones(cv::Size(3, 3), CV_8UC1), cv::Point(-1, -1), 2);
                ...
                cv::imshow("Before Projection", erode_merge);

                //=============================
                // Projection Start
                //==============================
                
                for (int idx = 0; idx < img_erode.rows; idx++) {
                    int count = 0;
                    for (int jdx = 0; jdx < img_erode.cols; jdx++) {
                        if (img_erode.at<uchar>(idx,jdx) == 0)
                            count += 1;
                    }
                    if(count>= 200)
                        for (int jdx = 0; jdx < img_erode.cols; jdx++) {
                            img_erode.at<uchar>(idx,jdx) = 255;
                        }
                }

                for (int idx = 0; idx < img_erode.cols; idx++) {
                    int count = 0;
                    for (int jdx = 0; jdx < img_erode.rows; jdx++) {
                        if (img_erode.at<uchar>(jdx, idx) == 0)
                            count += 1;
                    }
                    if (count <= 20)
                        for (int jdx = 0; jdx < img_erode.rows; jdx++) {
                            img_erode.at<uchar>(jdx, idx) = 255;
                        }
                }
                ...
                cv::Mat Merge_projection[] = { img_erode,img_erode,img_erode };
                cv::Mat projection_merge;
                cv::merge(Merge_projection, 3, projection_merge);

                cv::imshow("After Projection", projection_merge);
